...
def my_softmax(values: np.ndarray, T=1.):
    ...
    """
    # your code here
    subtracted_values = values - np.max(values)
    exp_values = np.exp(subtracted_values / T)
...
class QLearningAgent:
    ...
    def get_best_action(self, state):
        ...
        q_values = [self.get_qvalue(state, x) for x in possible_actions]
...
class EVSarsaAgent(QLearningAgent):
    ...
    def get_value(self, state):
        ...
        value = 0
        p_all = self.get_softmax_policy(state)
        for i in range(len(possible_actions)):
            value += p_all[i] * self.get_qvalue(state, possible_actions[i])
        assert value is not None
